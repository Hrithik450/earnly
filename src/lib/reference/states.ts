/**
 * Subdivisions by country name, each list alphabetical.
 *
 * Deliberately not imported anywhere directly — reach it through `loadStates`
 * in ./countries, which imports it dynamically. A static import from a client
 * component would pull the whole map into that page's bundle and undo the
 * reason it is a separate file.
 *
 * Keyed by the same country strings as COUNTRIES, so the two cannot drift.
 * Countries with no subdivisions are absent rather than present-and-empty.
 */
